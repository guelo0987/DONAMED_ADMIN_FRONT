...
export function ListaCategoriasEnfermedades() {
    ...
    const [eliminarTarget, setEliminarTarget] = useState<Enfermedad | null>(null);
    ...
    const fetchEnfermedades = useCallback(async () => {
        ...
        setError(null);
        try {
            const data = await catalogoService.getEnfermedades();
            setEnfermedades(data);
        } catch (err) {
            setError(err instanceof Error ? err.message : "Error al cargar categorías de enfermedades");
            setEnfermedades([]);
        } finally {
            setIsLoading(false);
        }
    ...
    const filtered = enfermedades.filter((item) => {
        if (search.trim() === "") return true;
        const value = search.toLowerCase();
        return (
            item.nombre.toLowerCase().includes(value) ||
            String(item.idenfermedad).includes(value)
        );
    });
    ...
    const handleEliminar = async () => {
        ...
        try {
            await catalogoService.deleteEnfermedad(eliminarTarget.idenfermedad);
            setEnfermedades((prev) => prev.filter((e) => e.idenfermedad !== eliminarTarget.idenfermedad));
                        ...
                        {isLoading ? (
                            ...
                            <>
                                {filtered.map((item) => (
                                    ...
                                    >
                                        <span className="font-medium">{item.idenfermedad}</span>
                                        <span className="font-medium">{item.nombre}</span>
                                        <div className="flex items-center gap-2">
                                            <Link
                                                to={`/categorias-enfermedades/${item.idenfermedad}`}
                                            ...
                                            <Link
                                                to={`/categorias-enfermedades/${item.idenfermedad}/editar`}
